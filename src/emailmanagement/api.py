...
from emailmanagement.contact_graph import ContactGraph, RelationshipClass
...
executor = ActionExecutor(store=store, has_write_scope=True)

# Seed metrics with sample data
for _ in range(1362):
    metrics.record_incoming_message()
for _ in range(1284):
...
for cid, rcls, score, count, protected in _sample_contacts:
    graph._nodes[cid] = __import__(
        "emailmanagement.contact_graph", fromlist=["ContactNode"]
    ).ContactNode(
        id=cid,
